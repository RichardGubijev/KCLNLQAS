...
class docuemenet_reranker:
    ...
    def rerank(self, search_query, paragraphs) ->  list:
        ...
        scores_paragraphs = list(zip(sentences, scores))
        return scores_paragraphs

    def rerank_indicies(self, search_query, indicies) ->  list:
        paragraphs = []
        for i in indicies:
        ...
        similarity_scores = cosine_similarity(sentence_embeddings)
        scores = similarity_scores[0][:]
        scores_paragraphs = list(zip(indicies, scores))
        return scores_paragraphs
